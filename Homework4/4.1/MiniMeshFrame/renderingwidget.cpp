...
void RenderingWidget::minimalSurfaceLocal()
{
	if (!is_read_)
	{
		return;
	...
	double totaltime;
	start = clock();
	MinimalSurface minimalSuface(ptr_mesh_);
	ptr_mesh_ = minimalSuface.localWay();
	finish = clock();
	totaltime = (double)(finish - start) / CLOCKS_PER_SEC;
	cout << "Local方法的运行时间为：" << totaltime << "秒！" << endl;
	updateGL();
}


void RenderingWidget::minimalSurfaceGlobal()
{
	if (!is_read_)
	{
		return;
	...
	start = clock();
	MinimalSurface minimalSuface(ptr_mesh_);
	ptr_mesh_ = minimalSuface.globalWay();
	finish = clock();
	totaltime = (double)(finish - start) / CLOCKS_PER_SEC;
	cout << "Global方法的运行时间为：" << totaltime << "秒！" << endl;
	updateGL();
}