...
{
	//myaction_ = new QAction(tr("myaction_"), this);
	//connect(myaction_, SIGNAL(triggered()), this, SLOT(MyFunction()));

	action_uniform_ = new QAction(tr("Uniform"), this);
	connect(action_uniform_, SIGNAL(triggered()), renderingwidget_, SLOT(Uniform()));

	action_WLS_ = new QAction(tr("WeightedLeastSquares"), this);
	connect(action_WLS_, SIGNAL(triggered()), renderingwidget_, SLOT(WeightedLeastSquares()));

	action_shapePreserving_ = new QAction(tr("ShapePreserving"), this);
	connect(action_shapePreserving_, SIGNAL(triggered()), renderingwidget_, SLOT(ShapePreserving()));
...
{
	mymenu_ = menuBar()->addMenu(tr("mymenu_"));
	mymenu_->addAction(action_uniform_);
	mymenu_->addAction(action_WLS_);
	mymenu_->addAction(action_shapePreserving_);
...
{
	mytoolbar_ = addToolBar(tr("mytoolbar_"));
	mytoolbar_->addAction(action_uniform_);
	mytoolbar_->addAction(action_WLS_);
	mytoolbar_->addAction(action_shapePreserving_);
